  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    doAsync {
      ...
      if (workoutDatabase.exerciseDao().getAll().blockingFirst().isEmpty()) {
        ...
        val exerciseId = workoutDatabase.registrationDao().insert(PlannedExercisePojo(workoutId, exercises[0], 0)).blockingGet()
        workoutDatabase.setsDao().insert(PlannedSetPojo(8, 0, exerciseId)).blockingGet()
        workoutDatabase.setsDao().insert(PlannedSetPojo(8, 1, exerciseId)).blockingGet()
        val registration2Id = workoutDatabase.registrationDao().insert(PlannedExercisePojo(workoutId, exercises[1], 1)).blockingGet()
        workoutDatabase.setsDao().insert(PlannedSetPojo(8, 0, registration2Id)).blockingGet()
        workoutDatabase.setsDao().insert(PlannedSetPojo(8, 1, registration2Id)).blockingGet()
      }
