...
abstract class AbstractAddItemAdapter<T> : AbstractBindableAdapter<T>(), AddItemAdapter {
  companion object {
  ...
  final override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
    if (holder.itemViewType == ADD_ITEM_VIEW_TYPE) {
      (holder as AbstractBindableAdapter<*>.AddItemViewHolder).let { addItemViewHolder ->
